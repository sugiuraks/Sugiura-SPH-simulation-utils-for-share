...
class FileHeader{
 public:
  ...
  void writeBinary(FILE* fp) const{
    ...
    int temp=MAX_FLAW_NUMBER_OUTPUT;
...
class RealPtcl{
 public:
  ...
  PS::F64 eps_ij_act_flaw[MAX_FLAW_NUMBER_OUTPUT]; //array of flaw activation threshold strain
  ...
  void writeAscii(FILE* fp) const{
    double km=1.0e5;
    fprintf(fp, "%f %f %f %f %f %f %f \n", this->pos.x, this->pos.y, this->pos.z, this->dens, this->pres, this->Sab.xy, this->Sab.yz);
    //fprintf(fp,"%d %e %e %e %e %e %e %e \n",this->id,this->pos.x,this->vel.x,this->eng,this->Sab_rho.xy,this->Sab.xy,this->dens,this->snds);
    //fprintf(fp,"%.2f %.2f %.2f %.2f %.2f \n",this->pos.x/km,this->pos.y/km,this->pos.z/km,this->smth/km,this->damage);
  }
  ...
  void writeBinary(FILE* fp) const{
    ...
    fwrite(this->eps_ij_act_flaw,sizeof(PS::F64),MAX_FLAW_NUMBER_OUTPUT,fp);
  }
