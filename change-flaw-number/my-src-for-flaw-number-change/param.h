...
#define MAX_FLAW_NUMBER_OUTPUT 50 //max number of flaws for one SPH particle for output
...
namespace PARAM{
  ...
  const PS::F64 MINIMUM_DENSITY = 0.1; //minimum density in the case of time-evolved density
  const PS::F64 SEARCH_RADIUS_H = 4.0; //This value defines how far we search SPH particles. The search radius becomes this value times larger than h.
  ...
  const PS::S32 DENSITY_DEVELOPMENT_METHOD = 1;//Select density development method. If 0, we calculate density by summation. If 1, we follow the time evolution of density by EoC.
const PS::S32 IS_CORRECTED_VELOCITY_GRADIENT=1;//If this value is 1, we corrected velocity gradients for calculating drho/dt and dSab/rho/dt. Now,for dSab/rho/dt, this is only valid for SSPH.
  ...
  const PS::F64 Y0_COHESION[N_MATERIAL]={1.0e9}; //cohesion (yield stress for zero pressure)
  ...
  const PS::F64 PS_POR[N_MATERIAL]={2.0e9}; //pressure for complete compaction
  const PS::F64 PE_POR[N_MATERIAL]={1.0e7}; //threshold pressure between elastic and plastic regime
  ...
  const PS::F64 U_CRIT_RIEMANN[N_MATERIAL] = {3.04e10}; //threshold internal energy for Riemann solver between ideal and elastic eos
