...
#pragma omp parallel for private(i,d,d2,sigma_eff_i,a,b,c,p,q,alpha_plus,alpha_minus,sigma_1,sigma_2,sigma_3,sigma_max)
  ...
  for(i=0;i<particle_number;i++){
    ...
    p = ( 3.0*b - a*a ) / 9.0;
    alpha_plus = pow( -q + sqrt( (std::complex<double>)(q*q + p*p*p) ) , 1.0/3.0 );
    alpha_minus = pow( -q - sqrt( (std::complex<double>)(q*q + p*p*p) ) , 1.0/3.0 );
    sigma_1 = (alpha_plus + alpha_minus - (a/3.0)).real();
    sigma_2 = (0.5 * ( -1.0 + I*sqrt(3.0) ) * alpha_plus + 0.5 * ( -1.0 - I*sqrt(3.0) ) * alpha_minus - (a/3.0)).real();
    sigma_3 = (0.5 * ( -1.0 - I*sqrt(3.0) ) * alpha_plus + 0.5 * ( -1.0 + I*sqrt(3.0) ) * alpha_minus - (a/3.0)).real();
